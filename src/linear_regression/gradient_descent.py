...
def found_min(cost, prev_cost, threshold, max_iter, i):
    ...
    return diff is not None and diff < threshold and diff != 0 or (max_iter is not None and i >= max_iter)
...
def update_weights(rate, n, X, y, y_pred, weights):
    ...
    return [w - derivative(rate, n, X.T[i], y, y_pred) for i, w in enumerate(weights)]


def gradient_descent(X: np.ndarray, y, rate=0.05, threshold=1e-15, max_iter=None, verbose=False, pred=None):
